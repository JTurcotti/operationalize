...
The above information processing pipeline is meant to show how ``procedure-to-claim'' proofs decompose into a series of independently-verifiable algebraic transforms. This section does \textit{not} attempt to show how real world choices of $\R_{qry}$, $\R_{exp}$, and $\beta$ translate into particular claims inhabited by this proofs; for that, see section \ref{sec:app}.

Given that the user either accepts \textit{claim specifications} $(\R_{qry}, \R_{exp}, \beta)$ as given, or has internalized rationale for choosing them herself, the algebraic construction of the estimator $\gamma$ whose semantics define the ultimate claim proceeds as follows:
