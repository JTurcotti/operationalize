...
\item Fix a ``hard'' decision $\A$ defined by a mapping $\beta: \mathbb{P}_p[\D_{qry}] \to \A$ - i.e. pick a concrete (non-probabilistic) property of distributions over $\D_{qry}$.
...
\item[[3 $\to$ 4, 5]] Similarly to the $[1 \to 2]$ case, exact specification of $\gamma$ is algebraically complex, and computationally infeasible. Verifiction of algebraic rewrites or computational approximation is highly valuable.
\end{description}
