...
Instead of denotationalizing programs as specific instances of monadic computation, \textit{itrees} assume only the existence only of a \textit{free monad}\footnote{a monad with no properties except those common to all monads}, and constructs programs not as particular inductive instances of the monad, but as coinductive instances of procedures that progress\footnote{to an (optional) point of termination} through a mix of ``internal'' computation steps (i.e. computation that does not depend on the monad) and external ``interaction'' steps (that split the program into a prequel that terminates in a ``request'' to the monad, and a sequel continuation that depends on the ``result'' of that request).

Since the evaluation (destruction) of itrees is equivalent to the construction of free monads, itrees are often referred to as ``coninductive (variants of) free monads''. \footnote{The power of this duality arises from the polymorphism afforded by using free monads as an ``interface'' between a program and its environment; on the monad (environment) side, evaluation via a concrete computational monad relies on the program only through its semantics against the free monad; on the comonad (program) side, the semantics of a program that relies on nontrivial properties of the environment is always realizable as a refiement of its semantics operating against the free monad}
...
Since ITrees represent a program's interactions with the environment in an entirely syntactically distinct way from the other internal details of the program or the environment, equivalence up to environmental interactions (``equivalence up to Tau'') becomes much easier to reason about than in the general case in which details of a program, its interactions with the environment, and the environment are all entangled.

In particular, equivalence up to tau can often be derived from the composed application of simple rewrite rules. The rewrite rules can be justified in part from the free semantics of itrees, and in part from refining properties of the program and environment. Proofs that depend only on equivalence up to tau are thus stable in a very general way: any changes to the program or environment that do not invalidate the properties relied upon to justify the set of itree equivalence rewrite rules do not invalidate the proof.

\subsubsection{Applications of ITree Equivalence to \AIM Programs}

\newcommand{\OI}{\ensuremath{(\O, \mathcal{I})~}}

The environment in which \AIM programs operate is the universe/meta-universe pair $(\U, \MU)$. Yet the inference rules above defining operational semantics for \AIM program relies on $\U$ and $\MU$ only through the functions $\O$ and $\mathcal{I}$. This presents a natural refinement of a free probability monad to one that concretizes a causal graph, and exposes \OI such that the three laws of Pearl's do-calculus are respected. The semantics of \AIM programs thus reduce to the semantics of itrees against this \OI ``do-calculus'' signature.

Thus itrees reduce reasoning about the correspondence between particular causal models and particular causal experiments to seperate reasoning about the models and experiments, respectively. In particular, models only ever need be analyzed up to their ability to answer \OI queries, and programs only ever need be analyzed up to their ability to ask \OI queries.

As a powerful motivating example, consider the following theorem:

\begin{equation*}
  \A_Z\A_X\M_Y \equiv_\tau \A_Z\I_X\M_Y \iff \text{no-backdoor}(X \to Y \mid Z)
\end{equation*}
...
Our exposition of \AIM programs as coinductive instances of \OI--interaction makes clear that ``equivalence up to tau'' is the right choice of program equivalence to craft stable, minimal proofs of theorems like this.

Further expansion of this direction of analysis is a short-term research priority, and will be further reflected in this report as it progresses.
